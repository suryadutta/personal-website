...
def calculate_and_save_values(Msamp,Esamp,spin,num_analysis,index,temp,data_filename,corr_filename):
    try:
        ...
        C_V_array = []
        for i in range(50):
            bin_start = int(i*len(Msamp[-num_analysis:])/50)
            bin_end = int((i+1)*len(Msamp[-num_analysis:])/50)

            E_binned = Esamp[bin_start:bin_end]
            C_V = 1/(temp)*(np.square(np.std(E_binned)))
            C_V_array.append(C_V)
        CV_mean = np.average(C_V_array)
        CV_std = np.std(C_V_array)
        ...
        for i in range(50):
            ...
            Chi = np.square(np.std(M_binned))
            Chi_array.append(Chi)
        Chi_mean = np.average(Chi_array)
        Chi_std = np.std(Chi_array)

        data_array = [np.abs(M_mean),M_std,E_mean,E_std, CV_mean, CV_std, Chi_mean, Chi_std]

        #write data to CSV file
        header_array = ['Temperature','Magnetizatio n Mean','Magnetization Std Dev','Energy Mean','Energy Std Dev', 'CV_mean', 'CV_Std', 'Chi_mean', 'Chi_std']
...
def run_simulation(t_min,t_max,t_step,n,num_steps,num_analysis,num_burnin,j,b,flip_prop,data_filename,corr_filename,plots):
    ...
    for index, temp in enumerate(temp_range):
        ...
        try:
            #run the Ising model
            Msamp, Esamp, spin = run_ising(n,temp,num_steps,num_burnin,num_analysis, flip_prop,j,b,disable_tqdm=False)
